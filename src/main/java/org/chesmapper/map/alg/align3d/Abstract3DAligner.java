...
public abstract class Abstract3DAligner extends AbstractAlgorithm implements ThreeDAligner
{
	...
	public void alignToSmarts(DatasetFile dataset, List<ClusterData> clusters)
	{
		...
		if (Settings.CACHING_ENABLED && new File(alignedFile).exists())
		{
			...
			for (ClusterData cluster : clusters)
			{
				...
				String destFile = dataset.getAlignResultsPerClusterFilePath(count,
						cluster.getSubstructureSmarts(type) + matchEngine);
		...
		else
		{
			...
			for (ClusterData cluster : clusters)
			{
				...
				{
					destFile = dataset.getAlignResultsPerClusterFilePath(count,
							cluster.getSubstructureSmarts(type) + matchEngine);
					...
					{
						TaskProvider
								.update("3D align cluster " + (count + 1) + "/" + clusters.size()
			...
			for (ClusterData cluster : clusters)
			{
				...
				else
				{
					if (!cluster.isAligned())
						throw new IllegalStateException();
					SDFUtil.replaceCompounds(alignedFile, alignedFiles[i],
							cluster.getCompoundClusterIndices());
				}
				i++;
			}
		}
	...
	private boolean alignWithCDK(DatasetFile dataset, ClusterData cluster, int index,
			String destFile)
	{
		...
		try
		{
			...
			FeatureService.writeOrigCompoundsToSDFile(dataset.getCompounds(), destFile,
					compoundOrigIndices, true, false);
			return true;
		}
		catch (Exception e)
		{
			...
			String destFile)
	{
		...
		try
		{
			...
			SDFUtil.filter(dataset.getSDFClustered(), tmpFirst.getAbsolutePath(),
					new int[] { cluster.getCompoundClusterIndices().get(0) }, true);
			...
			SDFUtil.filter(dataset.getSDFClustered(), tmpRemainder.getAbsolutePath(),
					remainderIndices, true);
		...
		catch (ExternalToolError e)
		{
			...
			Settings.LOGGER.warn(
					"error occured, checking if smarts are matching in openbabel for debugging");
			ExternalToolUtil.run("match-ref",
					new String[] { BinHandler.BABEL_BINARY.getLocation(),
							tmpFirst.getAbsolutePath(), "-osmi", "--filter",
							"\"s='" + cluster.getSubstructureSmarts(type) + "'\"" });
			ExternalToolUtil.run("match-mve",
					new String[] { BinHandler.BABEL_BINARY.getLocation(),
							tmpRemainder.getAbsolutePath(), "-osmi", "--filter",
							"\"s='" + cluster.getSubstructureSmarts(type) + "'\"" });
